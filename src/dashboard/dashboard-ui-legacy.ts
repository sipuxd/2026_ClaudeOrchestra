  ...
  <div class="team-detail" id="teamDetail" style="display:none">
    ...
    <div class="controls-bar" id="controlsBar">
      <div class="controls-actions" id="controlsActions">
        ...
        <button class="btn btn-merge" id="createPrBtn" onclick="createPr()" style="display:none">Create PR</button>
        <span id="prStatus" style="display:none"></span>
...
const PHASE_LABELS = { pre_work: 'Scan', work: 'Build', handoff: 'Sweep', review: 'Review', done: 'Done', pr_open: 'PR Open', merged: 'Merged' };
const PHASE_COLORS = { pre_work: '#58a6ff', work: '#7ee787', handoff: '#f0c55b', review: '#d2a8ff', done: '#7ee787', pr_open: '#a371f7', merged: '#7ee787', errored: '#f85149', cancelled: '#484f58' };
...
evtSource.addEventListener('pr-created', (e) => {
  const data = JSON.parse(e.data);
  if (teams[data.teamId]) {
    teams[data.teamId].currentPhase = 'pr_open';
    teams[data.teamId].prNumber = data.prNumber;
    teams[data.teamId].prUrl = data.prUrl;
  }
  renderSidebar();
  if (selectedTeamId === data.teamId) renderControlsBar();
});

evtSource.addEventListener('team-archived', (e) => {
  const data = JSON.parse(e.data);
  const prUrl = data.prUrl || '#';
  showNotification('Team archived — PR merged &#10003; <a href="' + prUrl + '" target="_blank" rel="noopener" style="color:#a371f7">View PR</a>', 'success');
  if (teams[data.teamId]) {
    delete teams[data.teamId];
  }
  delete agentOutputs[data.teamId];
  delete agentStreaming[data.teamId];
  delete agentSubtasks[data.teamId];
  delete feedbackItems[data.teamId];
  delete securityReviewState[data.teamId];
...
function renderSidebar() {
  ...
  const terminal = ['done', 'errored', 'cancelled', 'merged'];
...
function renderControlsBar() {
  if (!selectedTeamId || !teams[selectedTeamId]) return;
  const phase = teams[selectedTeamId].currentPhase;
  const terminal = ['done', 'errored', 'cancelled', 'pr_open', 'merged'];
  ...
  // Create PR button — visible when phase is done
  const createPrBtn = document.getElementById('createPrBtn');
  createPrBtn.style.display = phase === 'done' ? '' : 'none';
  createPrBtn.disabled = false;
  createPrBtn.textContent = 'Create PR';

  // PR status — visible when phase is pr_open
  const prStatus = document.getElementById('prStatus');
  if (phase === 'pr_open') {
    const team = teams[selectedTeamId];
    const prNum = team.prNumber || '?';
    const prUrl = team.prUrl || '#';
    prStatus.style.display = '';
    prStatus.innerHTML = '<a href="' + prUrl + '" target="_blank" rel="noopener" style="color:#a371f7;text-decoration:none;font-weight:600">PR #' + prNum + ' Open &#8599;</a>';
  } else {
    prStatus.style.display = 'none';
    prStatus.innerHTML = '';
  }

  // Preview button — visible when phase is done or pr_open
  document.getElementById('previewBtn').style.display = (phase === 'done' || phase === 'pr_open') ? '' : 'none';
...
async function createPr() {
  if (!selectedTeamId) return;
  if (!confirm('Create a Pull Request for "' + selectedTeamId + '"?\\n\\nThis will push the team branch and open a PR on GitHub.')) return;
  ...
  try {
    const res = await fetch('/api/teams/' + encodeURIComponent(selectedTeamId) + '/create-pr', { method: 'POST' });
    const data = await res.json();
    if (data.success) {
      showNotification('PR created: <a href="' + (data.prUrl || '#') + '" target="_blank" rel="noopener" style="color:#a371f7">#' + (data.prNumber || '') + '</a>', 'success');
      btn.style.display = 'none';
    } else {
      showNotification('PR creation failed: ' + (data.output || data.error || 'Unknown error'), 'error');
